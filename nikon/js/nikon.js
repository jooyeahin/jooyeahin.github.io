$(document).ready(function(){
  ...
  {/* <input type="checkbox" class="allchk" checked="checked">
  ...
  $('.allchk').change(function(){
    if($(this).prop('checked')){
      ...
      $('.prlist li').each(function(i){
        listprice=$(this).find('.price').text().replace(/[^0-9]/g,'');
        //console.log(i+':', listprice)
        priceSum +=Number(listprice);
      })
      //console.log(priceSum);
      $('.total-price').text('KRW  ' + priceSum.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
