$(document).ready(function(){
  ...
  $(window).scroll(function(){
    ...
    console.log(windowH)
  	...
  	if(windowH>3314){
      $('.metalic-back').css({'display':'block'});
    }else{
  		$('.metalic-back').css({'display':'none'});
  	}
  	var texturetop = windowH-3803;
  	if(windowH>3314){
  			$('.metalic-back > li:eq(0)').css({'height':(973-texturetop)+'px'})
  			$('.metalic-back > li:eq(1)').css({'height':(1462-texturetop)+'px'})
  			$('.metalic-back > li:eq(2)').css({'height':(1952-texturetop)+'px'})
  			$('.metalic-back > li:eq(3)').css({'height':(2441-texturetop)+'px'})
  	}
  });
